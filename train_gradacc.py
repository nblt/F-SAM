...
parser.add_argument('--grad-acc', default=1, type=int,
                    metavar='A', help='gradient accumulation (default: 1)')
...
def train(train_loader, model, criterion, optimizer, lr_scheduler, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        args.steps += 1
        if args.steps % args.grad_acc == 0:
            if args.accumulated_grad is not None:
                args.accumulated_grad += get_model_grad_vec(model)
                update_grad(model, args.accumulated_grad)
            optimizer.second_step(zero_grad=True)
            args.accumulated_grad = None
        else:
            if args.accumulated_grad is None:
                args.accumulated_grad = 0
            args.accumulated_grad += get_model_grad_vec(model)
